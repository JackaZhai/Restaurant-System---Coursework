...
void connectToDatabase_cooker(MYSQL *&conn); // 连接到数据库
void displayPendingOrders(MYSQL *conn); // 显示所有待做订单
void updateOrderStatus_cooker(MYSQL *conn, int order_id, int status); // 更新订单状态
...
void displayPendingOrders(MYSQL *conn) {
    ...
    cout << "\n==================== 待做订单 ====================\n";
    cout << left << setw(10) << "订单ID" << setw(10) << "桌号" << setw(20) << "菜品名称" << setw(10) << "数量" << setw(10) << "总价"
            << endl;
    cout << "-------------------------------------------------\n";

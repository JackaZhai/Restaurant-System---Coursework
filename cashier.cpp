...
void settleBill(MYSQL *conn, int table_id) {
    string query = "SELECT dish_name, price, quantity, total_price FROM orders WHERE table_id = " + to_string(table_id) + " AND status = 2";
...
void dailyStatistics(MYSQL *conn) {
    ...
    cout << "----------------------\n";
    while ((row = mysql_fetch_row(res))) {
        cout << left << setw(10) << (row[0] ? row[0] : "NULL") << setw(10) << (row[1] ? row[1] : "NULL") << " 元" << endl;
    }
    cout << "======================\n";
...
void dailyDishStatistics(MYSQL *conn) {
    ...
    while ((row = mysql_fetch_row(res))) {
        ...
        cout << left << setw(10) << (row[0] ? row[0] : "NULL") << setw(20) << (row[1] ? row[1] : "NULL") << setw(10) << status << setw(10) << (row[3] ? row[3] : "NULL") << endl;
    }
    cout << "======================\n";
